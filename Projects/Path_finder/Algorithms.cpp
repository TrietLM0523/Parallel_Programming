...
void Dijkstra(Graph &g, const std::string &startNode, const std::string &endNode)
{
    ...
    for (std::string at = endNode; at != ""; at = previous[at])
        path.push_back(at);
...
void AStar(Graph &g, const std::string &startNode, const std::string &endNode)
{
    ...
    // In đường đi của A*
    if (gScore[endNode] == std::numeric_limits<int>::max())
        return;
    std::vector<std::string> path;
    for (std::string at = endNode; at != ""; at = previous[at])
        path.push_back(at);
...
void flushCPUCache()
{
    // Tạo một mảng khổng lồ 32MB (khoảng 8 triệu số int, mỗi int 4 bytes)
    const int CACHE_SIZE = 8 * 1024 * 1024;
    std::vector<int> trash_data(CACHE_SIZE, 0);

    // Bắt CPU ghi dữ liệu rác vào từng ô nhớ
    for (int i = 0; i < CACHE_SIZE; ++i)
    {
        trash_data[i] = i;
    }

    // Ép CPU phải thực sự đọc lại đống rác đó (dùng volatile để trình biên dịch
    // không tự ý xóa vòng lặp này đi trong lúc tối ưu hóa code)
    volatile int sum = 0;
    for (int i = 0; i < CACHE_SIZE; ++i)
    {
        sum += trash_data[i];
